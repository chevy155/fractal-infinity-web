import { buildDetailHtml, classTag, downstream, evidenceFor, nodeById, upstream } from "./render.js";
const $ = (id) => document.getElementById(id);
async function load(path) { const response = await fetch(path); if (!response.ok) throw new Error(path); return response.json(); }
function neighborHtml(title, edges, direction, nodes) { if (!edges.length) return `<p class="edm-empty">No ${title.toLowerCase()} recorded at this depth.</p>`; return `<h3>${title}</h3><ul class="edm-neighbor-list">${edges.map((edge) => { const id = direction === "upstream" ? edge.to : edge.from; const node = nodeById(nodes, id); return `<li><button type="button" class="edm-neighbor-btn" data-id="${id}"><span class="edm-rel">${edge.rel.replaceAll("_", " ")}</span> <strong>${node?.label || id}</strong><span class="edm-rel-why">${edge.why}</span>${classTag(edge.class)}</button></li>`; }).join("")}</ul>`; }
async function main() {
  ...
  const ctx = { investigation, nodes: investigation.nodes, edges: edgesFile.edges, evidence: evidenceFile.evidence, concentration: concentrationFile.signals, trail: [investigation.anchor], onSelect: null };
  let mode = "both";
  ...
  $("scm-intel").innerHTML = (await import("./render.js")).buildIntelligenceHtml(intelligence); render(investigation.anchor, "both", true);
}
